...
def main():
    ...
    input_options.add_argument('source', nargs='?',
    ...
    formatgroup.add_argument('--markdown', action='store_true',
                             help='outputs formatted markdown to file')
    ...
    value_options.add_argument('-c', '--cache', dest='cache', metavar='PATH',
                               help='specify a primary cache directory to write to; default: try [ %s ]' % ', '.join(
                                   xml2rfc.CACHES))
    ...
    v2v3options.add_argument('--draft-revisions', '--draft-revs', action='store_true', default=False,
                             dest='draft_revisions',
    ...
    if pathlib.Path(source).is_dir():
        files = pathlib.Path(source).iterdir()
    else:
        files.append(source)
    ...
    #
    num_formats = len([o for o in [options.raw, options.text, options.nroff, options.markdown, options.html,
                                   options.expand, options.v2v3, options.preptool, options.info, options.pdf,
                                   options.unprep] if o])
    ...
    for file in files:
        ...
        parser = xml2rfc.XmlRfcParser(file,
                                      options=options,
        ...
        try:
            ...
            xml2rfc.log.exception('Unable to parse the XML document: ' + file.name, e)
            sys.exit(1)
            ...
            xml2rfc.log.exception('Unable to parse the XML document: ' + file.name, e.error_log)
            sys.exit(1)
        # check doctype
        ...
        try:
            source_path, source_base = os.path.split(file)
            ...
            if options.output_path:
                if os.path.isdir(options.output_path):
                    ...
                    sys.exit("The given output path '%s' is not a directory, cannot place output files there" % (
                        options.output_path,))
            else:
            ...
            if options.markdown:
                ...
                try:
                    ...
                    xml2rfc.log.write(e)
                    xml2rfc.log.write('Unable to complete processing %s' % file.name)
                    continue
                else:
                    if xmlrfc.tree:
                        writer = xml2rfc.MarkdownWriter(xmlrfc, options=options, date=options.date)
                        writer.write(filename)
                        options.output_filename = None
