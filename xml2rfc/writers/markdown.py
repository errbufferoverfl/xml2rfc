...
wrapper = utils.TextWrapper(width=72)  # width=72
...
def indent(text, indent=3, hang=0):
    ...
    # Replace Unicode Character 'LINE SEPARATOR' (\u2028) with a new line.
    text = text.replace('\u2028', '\n')
    for l in text.split('\n'):
...
def mktext(arg):
    """
    Handles line breaks in ordered & unordered lists and _some_ text blocks.
    """
    if isinstance(arg, six.text_type):
        text = arg
    else:
...
class MarkdownWriter(BaseV3Writer):

    def __init__(self, xmlrfc, quiet=None, options=default_options, date=None):
        super(MarkdownWriter, self).__init__(xmlrfc, quiet=quiet, options=options, date=date)
    ...
    def process(self):
        if not self.rendered:
            joiners = base_joiners
                ...
                # TODO (bec) changed the width to something obscenely wide until I can workout how to ignore it
            lines = self.render(self.root, width=10000, joiners=joiners)
            ...
            # Controls writing lines to a length of 72 characters
    ...
    def render_artwork(self, e, width, **kwargs):
        msg = ("(Artwork only available as %s: %s)"
               % (e.get('type', '(unknown type)'),
                  e.get('originalSrc') or e.get(
                      'src') or 'No external link available, see %s.html for artwork.' % self.root.get('docName')))
        ...
        # NOTE (errbufferoverfl) this is where you add the code fences
        lines.insert(0, Line(f"```{e.get('type', 'text')}", e))
        lines.append(Line("```", e))
        return lines
    ...
    def render_aside(self, e, width, **kwargs):
        ...
        # NOTE (errbufferoverfl) This is where you prefix the lines with a >
        prefix = '> '
        width -= len(prefix)
        ...
        for i, l in enumerate(lines):
            ...
            # NOTE (errbufferoverfl) This is where you remove any leading note annotations.
            # TODO (errbufferoverfl) It might make sense to generate callouts using any leading note annotations that
            #  way if RFCs ever support "Tip", "Hint", etc. they are captured.
            lines[i].text = prefix + l.text.replace("*Note:* ", "")
        lines = lindent(lines, indent=kwargs.get('indent', 0))
        
        # NOTE (errbufferoverfl) This is where you start an Obsidian formatted aside
        lines.insert(0, Line("> [!note]", e))
        return lines
    ...
    def render_bcp14(self, e, width, **kwargs):
        # NOTE (errbufferoverfl) this is where you updated bcp14 notation to be bold.
        return (f'**{e.text}**' or '') + (e.tail or '')
    ...
    def render_eref(self, e, width, **kwargs):
        # NOTE (errbufferoverfl) this is where many of the internal URLs associated with BCPs are processed for example:
        # [[BCP35]] at <https://www.iana.org/assignments/uri-schemes/> or
        # <xref target="BCP35"/> at
        #    <eref target="https://www.iana.org/assignments/uri-schemes/" brackets="angle"/>
        # For now I've decided to leave this as is because the eref doesn't have a strong association with the xref.
        target = e.get('target', '')
    ...
    def render_figure(self, e, width, **kwargs):
        ...
        if len(children) and children[0].tag == 'name':
            name = children[0]
            children = children[1:]
            title = self.tjoin(title, name, 90, **kwargs)
        lines = []
        for c in children:
            lines = self.ljoin(lines, c, width, **kwargs)

        # NOTE (errbufferoverfl) this is where you removed the addition of white space on table figures
        lines += mklines('_' + title.rstrip(stripspace) + '_', e)
        return lines
    ...
    def render_front(self, e, width, **kwargs):
        if e.getparent().tag == 'reference':
            ...
            # NOTE (errbufferoverfl) This is where you remove all the additional new lines from the top of the front 
            # page
            text = self.render_first_page_top(e, width, **kwargs) + '\n'
            lines = mklines(text, e)
            for c in e.getchildren():
    ...
    def render_first_page_top(self, e, width, **kwargs):
        def join_cols(left):
            # NOTE (errbufferovefl) this is where we remove the joining of the left and right column because we 
            # maintain a one column layout.
            return '\n'.join(left).rstrip(stripspace) + '\n'

        #
        def generate_obs_frontmatter(front):
            # left_parts = ['source', 'seriesInfo', 'obsoletes', 'updates', 'category', 'issn', 'expires', 'workgroup' ]
            frontmatter = ["---"]
            if self.root.get('ipr') == 'none':
                for group in front.xpath('./workgroup'):
                    if group.text and group.text.strip(stripspace):
                        found = True
                        frontmatter.append(group.text.strip(stripspace))
                return frontmatter

            frontmatter.append('title: ' + '"' + self.render_title_front(e.find('./title'), width, **kwargs) + '"')
            frontmatter.append('date: ' + self.render_date(front.find('./date'), width, **kwargs))
            if self.options.rfc:
                # 
                ...
                key = 'stream: '
                frontmatter.append('%s "%s"' % (key, strings.stream_name[stream]))
                #
                ...
                for item in front.iter('seriesInfo'):
                    ...
                    if name == 'RFC':
                        name = 'rfc'
                    frontmatter.append('%s: "%s"' % (name, value))
                ...
                if obsoletes:
                    frontmatter.append('obsoletes: ' + "[" + obsoletes + "]")
                updates = self.root.get('updates')
                if updates:
                    frontmatter.append('updates: ' + "[" + updates + "]")
                ...
                if category:
                    if category in strings.category_name:
                        frontmatter.append('category: "%s"' % (strings.category_name[category],))
                    else:
                        self.warn(self.root, "Expected a known category, one of %s, but found '%s'" % (
                            ','.join(strings.category_name.keys()), category,))
                else:
                    self.warn(self.root, "Expected a category, one of %s, but found none" % (
                        ','.join(strings.category_name.keys()),))
                #
                frontmatter.append('issn: 2070-1721')
                #
            else:
                # Internet-Draft
                found = False
                for group in front.xpath('./workgroup'):
                    if group.text and group.text.strip(stripspace):
                        found = True
                        frontmatter.append('workgroup: ' + group.text.strip(stripspace))
                if not found:
                    frontmatter.append('workgroup: ' + 'Network Working Group')
                frontmatter.append('status: ' + 'Internet-Draft')
                #
                category = self.root.get('category', '')
                #
                ## The following code duplicates series info also generated from <seriesInfo>
                ...
                if self.root.get('ipr') != 'none':
                    ...
                    frontmatter.append(
                        'expires: %s' % format_date(exp.year, exp.month, exp.day, self.options.legacy_date_format))
        ...
        def get_left(front, right=""):
            ...
            if self.options.rfc:
                ...
                #       may appear in future RFCs.
                obsoletes = self.root.get('obsoletes')
                # NOTE (errbufferoverfl) this is where you updated obsoletes so it doesn't wrap.
                if obsoletes:
                    left.append('Obsoletes: ' + obsoletes)
                updates = self.root.get('updates')
                if updates:
                    left.append('Updates: ' + updates)
                ...
                obsoletes = self.root.get('obsoletes')
                if obsoletes:
                    left.append('Obsoletes: ' + obsoletes + ' (if approved)')
                updates = self.root.get('updates')
                if updates:
                    left.append('Updates: ' + updates + ' (if approved)')
            ...
            if not right:
                # NOTE (errbufferoverfl) this is where we update the parser to handle author rendering and year 
                # rendering in a single column layout.
                auth = namedtuple('author', ['name', 'org'])
                authors = front.xpath('./author')
                ...
                left.append("\n")
                left.append(self.render_date(front.find('./date'), width, **kwargs))
            return left

        # TODO (errbufferoverfl) remove or make optional the rendering of right aligned content.
        def get_right(front):
            ...
            # NOTE (errbufferoverfl) this is where authors are rendered
            right = []
            auth = namedtuple('author', ['name', 'org'])
            prev = auth(None, '')
            authors = front.xpath('./author')
        ...
        # NOTE (errbufferoverfl) this is where you disable the right formatting
        # right = get_right(e)
        left = generate_obs_frontmatter(e)
        #
        first_page_header = join_cols(left)
        
        # NOTE (errbufferoverfl) This is where you add the first heading and tidy up all the additional whitespace 
        # rendered because of the obscene line length
        first_page_header += '\n# '
    ...
    def render_ol(self, e, width, **kwargs):
        ...
        if len(type) > 1:
            ...
            if formspec:
                ...
                self.err(e,
                         "Expected an <ol> format specification of '%%' followed by upper- or lower-case letter, of one of c,d,i,o,x; but found '%s'" % (
                             type,))
                fchar = 'd'
                e._format = '%s'
        else:
    ...
    def render_reference(self, e, width, **kwargs):
        ...
        if p.tag == 'referencegroup':
            ...
            # NOTE (errbufferoverfl) this is where we modify the bottom references so that they act as extended 
            # markdown footnotes
        ...
        # NOTE (errbufferoverfl) this is where you removed the padding for the references
        # text = indent(text, 11, 0)
        if p.tag != 'referencegroup':
            # NOTE (errbufferoverfl) this is where you disabled the addition of a new line after some bottom references
    ...
    def render_referencegroup(self, e, width, **kwargs):
        kwargs['joiners'].update({
            'reference': Joiner(' ', 0, 0, False, False),
            't': Joiner(' ', 0, 0, False, False),
        })
        label = self.refname_mapping[e.get('anchor')]

        # NOTE (errbufferoverfl) this is where we modify the bottom reference groups (i.e. the BCPs) so that they act
        # as extended markdown footnotes
        ...
        if lines[-1].elem.tag == "t":
            # NOTE (errbufferoverfl) Super hacky way to check if the last line is a link and to merge it onto the 
            # line above
            lines[-2].text = lines[-2].text + " " + lines[-1].text
            del lines[-1]
    ...
    def render_references(self, e, width, **kwargs):
        ...
        lines = []
        # NOTE (errbufferoverfl) this is where we disable the generation of the reference headers because in obsidian
        # all footnotes are moved to the very bottom

        # if e.find('name') != None:
        #     if self.check_refs_numbered():
        #
        #         # NOTE (errbufferoverfl) this is where the reference header is processed
        #         pn = e.get('pn')
        #         h_level = len(pn.split('-', 1)[1].replace('-', ' ').title().split(".")) + 1
        #         text = pn.split('-', 1)[1].replace('-', ' ').title() + '.'
        #         text = ' '.rjust(h_level, '#') + text
        #     else:
        #         text = ''
    ...
    def render_section(self, e, width, **kwargs):
        ...
        if e.get('numbered') == 'true':
            ...
            # NOTE (errbufferoverfl) this is where you work out the heading level when numbered
            h_level = len(num.split(".")) + 1
            text = num.title() + '.'
            ...
            # NOTE (errbufferoverfl) if numbered is false just assume level 2.
            text = "## " + text
        lines = []
        name = e.find('name')
    ...
    def render_sourcecode(self, e, width, **kwargs):
        ...
        if markers == 'true':
            # NOTE (errbufferoverfl) This is where we remove the leading and training <CODE BEGIN> notices because in
            # Markdown we can use normal code fences which are added later in the piece.
            #text = '<CODE BEGINS>'
            text = ''
            file = e.get('name')
    ...
    def build_table(self, e, width, **kwargs):
                ...
                for r in list(p.iterchildren('tr')):
                    ...
                    for c in r.iterchildren('td', 'th'):
                        ...
                        if cell.foldable:
                            ...
                            cell.minwidth = max([0] + [len(word.strip(stripspace)) for word in
                                                       splitter._split(cell.text)]) if cell.text else 0
                        else:
                            cell.minwidth = max(
                                [0] + [len(word.strip(stripspace)) for line in cell.text.splitlines() for word in
                                       splitter._split(line)])
                    i += 1
            ...
            for r in list(p.iterchildren('tr')):
                ...
                for c in r.iterchildren('td', 'th'):
                    ...
                    if len(c) == 1 and c[0].tag == 't':
                        cell.text, cell.foldable = self.text_or_block_renderer(c[0], width, fill=False, **kwargs) or (
                            '', True)
                    else:
                        cell.text, cell.foldable = self.text_or_block_renderer(c, width, fill=False, **kwargs) or (
                            '', True)
                    cell.text = mktextblock(cell.text)
                    if cell.foldable:
                        cell.text = cell.text.strip(stripspace)
                        cell.minwidth = max(
                            [0] + [len(word) for word in splitter._split(cell.text)]) if cell.text else 0
                    else:
                        cell.minwidth = max(
                            [0] + [len(word) for line in cell.text.splitlines() for word in splitter._split(line)])
                    cell.type = p.tag
                    if c.tag == 'th':
                        cell.top = bchar['=']
                        cell.bot = bchar['=']
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    if hasattr(cell, 'origin'):
                        if cell.origin == (i, j):
                            w = sum([cells[i][k].colwidth for k in
                                     range(j, j + cell.colspan)]) + cell.colspan - 1 - cell.padding
                            r = cell.rowspan
                            ...
                            if cell.text:
                                if cell.foldable:
                                    ...
                                    cell.wrapped = [l.text for l in
                                                    self.text_or_block_renderer(cell.element, width=w, fill=True,
                                                                                **kwargs)[0]]
            ...
            for n in range(min(len(c.wrapped) for c in cells[i] if c.wrapped) - int(has_bot_border)):
                ...
                if prev_bottom_border_line:
                    if has_top_border:
                        line = lines[-1]
                        lines[-1] = Line(''.join(
                            merge_border(prev_bottom_border_line.text[c], line.text[c]) for c in range(len(line.text))),
                            line.elem)
                    else:
                        line = lines[-1]
        ...
        # NOTE (errbufferoverfl) this is where we pop the top and bottom table headers so it renders as a markdown table
        lines.pop(0)
        lines.pop(-1)
        return lines
    ...
    def render_table(self, e, width, **kwargs):
        ...
        if table_width < min_title_width:
            ...
            lines = align(lines, 'left', table_width)
        # NOTE (errbufferoverfl) This is where you removed the generation of additional whitespace after code blocks
        title = '_' + title.rstrip(stripspace) + '_'
    ...
    def render_title_front(self, e, width, **kwargs):
        ...
        # NOTE (errbufferoverfl) This is where you prevent the additional padding
    ...
    def render_toc(self, e, width, **kwargs):
        # NOTE (errbufferoverfl) this is where we disable toc processing in favour of an obsidian macro
        # lines = []
        # for c in e.getchildren():
        #     numbered = c.get('numbered')
        #     if not numbered == 'false':
    ...
    def render_ul(self, e, width, **kwargs):
        ...
        symbols = ('*', '-', 'o', '+') if not self.options.list_symbols else self.options.list_symbols
    ...
    def render_xref(self, e, width, **kwargs):
        ...
        if not section:
                ...
                if target in self.refname_mapping:
                    if format == 'none':
                        ...
                        if content:
                            ...
                            # NOTE (errbufferoverfl) This is where we make internal references Wikilinks for the 
                            # purposes of Obsidian markdown
                            text = "[[%s]][^%s]" % (reftext, reftext)
                else:
                    if format == 'none':
                        text = "%s" % exptext
                    else:
            ...
            # NOTE (errbufferoverfl) this is where we process xref's that point to external sections or appendixes
            # here we convert all the reftext links to extended markdown footnotes
            # e.g. <xref target="CACHING" section="7"/> -> Section 7 of [CACHING]
            if sform == 'of':
